# Retro Game Coders — Documentation

Welcome to the ***documentation*** companion to **[ide.retrogamecoders.com](https://ide.retrogamecoders.com)** — a browser-based environment for retro programming across multiple platforms and languages.

## What you will find here

| Section | Purpose |
|--------|---------|
| **[IDE](ide/getting-started.md)** | Getting started — workflow, shortcuts, platforms |
| **[TRSE](trse/index.md)** | Turbo Rascal and built-in method reference |
| **[Assembly](assembly/index.md)** | [6502](assembly/6502.md), [Z80](assembly/z80.md) starters |
| **[C](c/index.md)** | [cc65](c/cc65.md) starter |
| **[BASIC](basic/index.md)** | [RGC BASIC](basic/rgc-basic.md), [Commodore BASIC v2](basic/commodore-basic-v2.md), [XC-BASIC 3](basic/xc-basic-3.md) |



---

[MkDocs Material](https://squidfunk.github.io/mkdocs-material/) | 
[Maintaining this site](MAINTAINING.md)
