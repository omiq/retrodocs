...
Companion to the [map format spec](https://github.com/omiq/rgc-basic/blob/main/docs/map-format.md) and [Level authoring](level-authoring.md). Where the spec defines the JSON schema, this page defines the **gameplay vocabulary** — what to put inside the `obj` layer to build a Zelda-class RPG: spawns, doors, NPCs, loot, weapons, spells, traps, power-ups, McGuffins, enemies, AI, and attack patterns.
...
Full reference: [Graphics — HUD authoring](graphics-raylib.md#hud-authoring-panel-png-attribute-icons). Demo: [`examples/hud_demo.bas`](https://github.com/omiq/rgc-basic/blob/main/examples/hud_demo.bas) ([open in IDE](https://ide.retrogamecoders.com/?file=hud_demo.bas&platform=rgc-basic)).
...
- [Level authoring](level-authoring.md) — `MAPLOAD` vs BASIC builder, migration path.
- [Graphics (Raylib)](graphics-raylib.md) — sprites, screen modes, `OVERLAY` HUD plane.
- [Language reference](language.md) — `MAPLOAD` / `MAPSAVE` reference.
- [Network & buffers](network-and-buffers.md) — load `.json` levels over HTTP for hot-reload during play-testing.
