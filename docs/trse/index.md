...
**TRSE** (Turbo Rascal Syntax Error) is a desktop IDE and compiler for 8-bit (and more) systems, created by *[Nicolaas Groeneboom](https://lemonspawn.com/)* (also known as "leuat") . Programs are written in **Turbo Rascal** — a clean, Pascal-flavoured language that compiles directly to native machine code for your target platform.

!!! note "Desktop app"
    This section was developed based upon the **TRSE desktop IDE**. The browser-based web IDE at [ide.retrogamecoders.com](https://ide.retrogamecoders.com) is a separate tool, therefore there will be some differences — see [Getting started with the IDE](../ide/getting-started.md) for that.
