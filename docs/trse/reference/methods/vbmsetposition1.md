...
<h3>vbmSetPosition1( [x-byte], [y-byte] );</h3>
<ul>
...
between 0 and 7 which is the pre-shifted offset to use to correctly display a sprite.
</p>
...
<h3>Example</h3>
<code>
    // assume using 1 pixel pre-shifted sprites, therefore a total of 8 pre-shifted positions

    vbmSetPosition1( 35, 100 );
    vbmDrawSprite8( sprLeftSide, sprRightSide ); // 8x8 sprites need two character columns

    vbmSetPosition1( 100, 100 );
