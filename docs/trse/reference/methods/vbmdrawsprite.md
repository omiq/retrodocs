...
<h3>vbmDrawSprite( [address1], [address2] );</h3>
<ul>
...
<h3>Description</h3>
<p>
    A simple sprite routine to display an 8x16 sprite on the bitmap.
</p>
<p>
...
<code>
    ...
    begin
        ...
        vbmDrawSprite( spr8L0, spr8R0 );  // draw the sprite
        
        waitforraster( 0 ); // wait for raster

        vbmSetPosition1( x, y ); // position the sprite, 1 pixel increments
        vbmClearSprite( spr8L0, spr8R0 );  // erase the sprite     
